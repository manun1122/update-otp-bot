...
const ADMIN_USERNAME = "@rana1132"; // অ্যাডমিনের ইউজারনেম
...
// Get multiple numbers for a user
...
/******************** ADMIN ACTIONS ********************/
// নিচে সব অ্যাডমিন অ্যাকশন দেওয়া হলো – এগুলো আগের কোডের মতোই সম্পূর্ণ আছে। সংক্ষেপে দেওয়া হলো।
...
bot.action("admin_upload", async (ctx) => {
  ...
  await ctx.editMessageText("📤 *Upload Numbers*\n\nSelect service for the numbers:", { parse_mode: "Markdown", reply_markup: { inline_keyboard: serviceButtons } });
});
  ...
  for (const countryCode in numbersByCountryService) {
    ...
    for (const serviceId in numbersByCountryService[countryCode]) {
      ...
      if (count > 0) {
        ...
        buttons.push([{ text: `🗑️ ${countryCode}/${serviceId} (${count})`, callback_data: `admin_delete_confirm:${countryCode}:${serviceId}` }]);
      }
    }
    report += "\n";
  }
  buttons.push([{ text: "❌ Cancel", callback_data: "admin_cancel" }]);
  await ctx.editMessageText(report, { parse_mode: "Markdown", reply_markup: { inline_keyboard: buttons } });
});
...
bot.action(/^admin_delete_confirm:(.+):(.+)$/, async (ctx) => {
  ...
  await ctx.editMessageText(
    ...
    { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "✅ Yes, Delete", callback_data: `admin_delete_execute:${countryCode}:${serviceId}` }, { text: "❌ Cancel", callback_data: "admin_back" }]] } }
  );
});
...
bot.action(/^admin_delete_execute:(.+):(.+)$/, async (ctx) => {
  ...
  saveNumbers();
  await ctx.editMessageText(`✅ *Deleted Successfully*\n\n🗑️ Deleted ${count} numbers\n📌 Country: ${countryCode}\n🔧 Service: ${services[serviceId]?.name || serviceId}`, { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "🔙 Back to Admin", callback_data: "admin_back" }]] } });
});
...
bot.action("admin_list_services", async (ctx) => {
  ...
  await ctx.editMessageText(report, { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "🔙 Back", callback_data: "admin_back" }]] } });
});
...
bot.action("admin_users", async (ctx) => {
  ...
  await ctx.editMessageText(message, { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "🔄 Refresh", callback_data: "admin_users" }], [{ text: "🔙 Back", callback_data: "admin_back" }]] } });
});
...
bot.action("admin_logout", async (ctx) => {
  ...
  ctx.session.adminState = null;
  ctx.session.adminData = null;
  await ctx.editMessageText("🚪 *Admin Logged Out*\n\nYou have been logged out from admin panel.", { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "🔙 Back to Main Menu", callback_data: "user_back" }]] } });
});
    ...
    if (ctx.session.isAdmin && ctx.session.adminState) {
      ...
      switch (adminState) {
        ...
        case "waiting_broadcast": {
          ...
          ctx.session.adminState = null;
          await ctx.reply(`📢 *Broadcast Complete!*\n\n✅ Sent: *${sent}* users\n❌ Failed: *${failedBroadcast}* users\n\n📝 Total users: ${Object.keys(users).length}`, { parse_mode: "Markdown" });
          break;
        }
      }
    }
...
async function startBot() {
  try {
    ...
    console.log("   • Help now shows admin contact only");
    console.log("=====================================");
